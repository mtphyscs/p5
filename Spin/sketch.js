...
function draw() {
  ...
  if (frameCount < 5) {
    ...
    backShader.setUniform("u_campos", [
      0,
      0,
...
class Spinner {
  constructor() {
    ...
    for (let i = 0; i <= res; i++) {
      let pos = createVector(
        -this.wholewidth * 0.5 + (this.wholewidth / (res - 1)) * i,
        0,
        0
      );
  ...
  update() {
    ...
    this.thickness =
      height *
      0.03 *
      map(sin(radians(curTime * this.speed * speedscale)), -1.0, 1.0, 0.2, 1.0);
    this.spinnum = map(
      sin(radians(curTime * this.speed * speedscale)),
      -1.0,
      1.0,
      6,
      12
    ...
    for (let i = 0; i < this.points.length; i++) {
      let ang =
        (TWO_PI * this.spinnum * i) / this.points.length +
        radians(curTime * this.speed);
      let offsetang = (i / this.points.length) * TWO_PI * this.wavenum;
      let x =
        cos(ang) *
        this.radius *
        map(
          sin(offsetang + radians(curTime * this.shiftspeed * 0.5)),
          -1.0,
          1.0,
          0.2,
          1.0
